...
struct WebStream::impl : public StreamSockSetter
...
#else
	impl(const char* fileName) : m_server("8080"), m_block_ctor(true), m_wnd(NULL), str_fileName(fileName)
		{
		// start server thread
		m_thread = std::thread(&WebStream::impl::WaitForClients, this);
	...
	void WaitForClients()
		{
		auto ds = std::unique_ptr<DummyClientSock>();
		m_clients.push_back(std::move(ds));

		errno_t err = fopen_s(&m_file, str_fileName.c_str(), "w+");
		if (err != 0)
			{
			GetLastError();
			}
	...
	void SetStreamSocket(DummyClientSock& s) override
		{
		for (size_t i = 0; i < m_clients.size(); ++i)
			{
			if (m_clients[i].get() == &s)
...
#else
	std::unique_ptr<DummyClientSock> m_stream_client;  ///< video streaming socket
	std::vector<std::unique_ptr<DummyClientSock>> m_clients; // heap allocated objects to ensure that they never change addreess
#endif // __use_socket__
...
#else
void WebStream::SetFileEndPoint(const char* fileName)
	{
