...
#else
class DummyClientSock; // forward decl
...
class StreamSockSetter
	...
	public:
		virtual ~StreamSockSetter() = default;
		virtual void SetStreamSocket(DummyClientSock& s) = 0;
		// virtual void SetStreamSocket(DummyClientSock& s);
		virtual void Unblock() = 0;
		// virtual void Unblock();
	};
#endif // __use_socket__

// #define IDR_WebStreamHtml 101
...
#else
class DummyClientSock final
	{
	public:
		explicit DummyClientSock(SOCKET cs) : m_sock(INVALID_SOCKET)
			{
			}

		// non-assignable
		DummyClientSock(const DummyClientSock&) = delete;
		DummyClientSock& operator = (const DummyClientSock&) = delete;

		// non-movable (not safe due to threading)
		DummyClientSock(DummyClientSock&& other) = delete;
		DummyClientSock& operator = (DummyClientSock&& other) = delete;

		void ConnectionThread(HWND wnd, StreamSockSetter* parent)
			{
			Connect type = Connect::CONTINUE;
			do {
				type = Handshake(wnd);
				} while (type == Connect::CONTINUE);
		...
		void Start(HWND wnd, StreamSockSetter* parent)
			{
			m_thread = std::thread(&DummyClientSock::ConnectionThread, this, wnd, parent);
			}
...
#else
		std::unique_ptr<DummyClientSock> WaitForClient()
			{
			return std::make_unique<DummyClientSock>((SOCKET)0);
			}
#endif // __use_socket__
